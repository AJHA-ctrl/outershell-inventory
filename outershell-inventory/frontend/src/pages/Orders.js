...
export default function Orders({ branch }) {
  ...
  const [form, setForm] = useState({ product:'', quantity:1, branch:'Andheri', customerName:'' });

  useEffect(() => {
    let o = branch === 'all' ? [...mockOrders] : mockOrders.filter(x => x.branch === branch);
    if (filterStatus !== 'all') o = o.filter(x => x.status === filterStatus);
    setOrders(o);
  }, [branch, filterStatus]);

  const counts = STATUSES.reduce((acc,s) => ({...acc,[s]:mockOrders.filter(o=>o.status===s).length}),{});

  return (
    <div className="fade-in">
      <div style={{ display:'flex', gap:6, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        {['all',...STATUSES].map(s => (
          <div key={s} onClick={()=>setFilterStatus(s)} style={{ padding:'5px 14px', borderRadius:20, fontSize:12, cursor:'pointer', fontWeight:filterStatus===s?500:400, background:filterStatus===s?'#185FA5':'#fff', color:filterStatus===s?'#fff':'#64748b', border:'0.5px solid #e2e8f0' }}>
            {s==='all'?'All Orders':s} {s!=='all'&&counts[s]>0?`(${counts[s]})`:''}
          </div>
        ))}
        <button style={{ ...btn(true), marginLeft:'auto' }} onClick={()=>setShowModal(true)}><i className="ti ti-plus"></i> New Order</button>
      </div>
      ...
      <div style={{ background:'#fff', border:'0.5px solid #e2e8f0', borderRadius:12, padding:16 }}>
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr>
              {['Order ID','Product','Branch','Customer','Qty','Amount','Status'].map(h=>(
            ...
            {orders.map(o => {
              ...
              return (
                <tr key={o._id} onMouseEnter={e=>e.currentTarget.style.background='#f8fafc'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                  ...
                  <td style={{ padding:10 }}><span style={{ background:s.bg, color:s.color, fontSize:11, fontWeight:500, padding:'2px 10px', borderRadius:20 }}>{o.status}</span></td>
                </tr>
              );
            })}
          </tbody>
        </table>
        {orders.length === 0 && <div style={{ textAlign:'center', padding:40, color:'#64748b', fontSize:13 }}>No orders found</div>}
      </div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.3)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100 }}>
          <div style={{ background:'#fff', borderRadius:12, padding:24, width:460, maxWidth:'95vw' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <span style={{ fontSize:15, fontWeight:500 }}>New Purchase Order</span>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <select style={inp} value={form.product} onChange={e=>setForm({...form,product:e.target.value})}>
                ...
                {mockProducts.filter(p=>p.stock>0).map(p=><option key={p._id} value={p._id}>{p.name} ({p.branch}) — {p.stock} left</option>)}
              </select>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
              <div>
                <label style={{ fontSize:12, color:'#64748b', marginBottom:5, display:'block' }}>Quantity *</label>
                <input style={inp} type="number" value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})} />
              </div>
              <div>
            ...
            <div style={{ marginBottom:16 }}>
              <label style={{ fontSize:12, color:'#64748b', marginBottom:5, display:'block' }}>Customer Name</label>
              <input style={inp} placeholder="Walk-in Customer" value={form.customerName} onChange={e=>setForm({...form,customerName:e.target.value})} />
            </div>
            <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
              <button style={btn(false)} onClick={()=>setShowModal(false)}>Cancel</button>
              <button style={btn(true)} onClick={()=>{ toast.success('Order placed! (Demo mode)'); setShowModal(false); }}>Place Order</button>
            </div>
          </div>
        </div>
      )}
